...
function addMoneyStatementTableHead() {
	$("#momeyresulthead").find("tr>th").remove();
	...
	$("#momeyresulthead").find("tr").replaceWith(newth);
}

function queryMoneyStatement() {
	...
	showMoneyStatementByQueryCondition(userName, beginTime, endTime);
}

function showMoneyStatementByQueryCondition(userName, beginTime, endTime) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			addMoneyStatementToDataTable(data.count, data.list);
...
function addMoneyStatementToDataTable(count, list) {
	$("#moneyresultbody").find("tr>td").remove();
	...
	for (var index = 0; index < list.length; index++) {
		...
		$("#moneyresultbody").find("tr:last").after(newRow);
	}
