...
$(function() {
	addUserDealStatementTableHead();
	initValidate();
	queryUserDealStatement();
...
function addUserDealStatementTableHead() {
	$("#userdealresulthead").find("tr>th").remove();
	$("#userdealresultbody").find("tr>td").remove();
	...
	$("#userdealresulthead").find("tr").replaceWith(newth);
}

function queryUserDealStatement() {
	...
	showUserDealStatementByQueryCondition(userName, beginTime, endTime);
}

function showUserDealStatementByQueryCondition(userName, beginTime, endTime) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			addUserDealStatementToDataTable(data.count, data.list);;
...
function addUserDealStatementToDataTable(count, list) {
	$("#userdealresultbody").find("tr>td").remove();
	...
	for (var index = 0; index < list.length; index++) {
		...
		$("#userdealresultbody").find("tr:last").after(newRow);
	}
